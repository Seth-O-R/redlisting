...
# make.boundary 
make.boundary <- function(occurences){
  # make points spatial
...
make.aoh.map <- function(occurences, the_aoh, T){
  ...
  # producing AOH map
    leaflet() %>%
      ...
      addCircleMarkers(data = points_spat, 
                       ...
                       stroke = F, 
                       fillOpacity = 0.8) %>%
      addPolygons(data = boundary_object, # change this depending on range source
                  color = "black",
                  weight = 1,
                  fillColor = "yellow",
                  group = "shape") %>% 
